...
void longest_closed ( Mat& mat ) {
  ...
  // rebel set - several sample pics fail with missing sides and a lot of noise
  ...
  // int canny_thresh_small = 1;
  // int appr_poly_epsilon = 40; /*10-smaller sides in noise shapes (more angles), 40-larger - larger poly-sides with less angles*/

    // original longest_closed set
  Point kernel_ksize(19,19); /*Size of the structuring element*/
  Size blur_ksize(10,10);
  int canny_thresh_big = 40; /*here*/
  int canny_thresh_small = 10;
  int appr_poly_epsilon = 40; /*10-smaller sides in noise shapes (more angles), 40-larger - larger poly-sides with less angles*/
