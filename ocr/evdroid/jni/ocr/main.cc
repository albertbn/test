...
void fork_loop_ocr ( Mat& im_orig, std::vector<Rect>& rect_lines ) {
	...
	// create pipe pair
	// int fd[2]; /*moved to be global var - in ../static_fields.hpp, implemented in the ../jni_java_native_bridge*/
	...
	if ( pid == 0 ) {
		...
		for ( int i=0; i<(int)rect_lines.size(); ++i ) {
			...
			crop_b_tess ( im_orig, rect_lines[i], i, fd ); /* implemented in ./tess.cc */
...
void ocr_doit ( Mat& im_orig ) {
	...
	fork_loop_ocr ( im_orig, rect_lines );
